...
`netfilum up` 仍然保留，但它只是本机开发时的快捷入口，会在 Windows 上代为拉起 WSL 里的 `netfilumd`，然后再执行挂载。
...
如果当前就是在 Windows + WSL 的同一台机器上开发，可以直接用：
...
1. 在指定的 WSL 发行版里启动 `netfilumd`
2. 等待 RPC 服务就绪
3. 在 Windows 上挂载盘符
4. 当前进程结束时卸载盘符并停止刚刚拉起的服务端

第一次运行如果触发了 `cargo run` 编译，盘符出现会慢一些。
