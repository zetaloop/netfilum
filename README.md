...
- `netfilum` 和 `netfilumd` 之间的 RPC 协议、请求分发、路径约束、文件语义映射，以及按密码启用的认证和基于 Argon2id 与 ChaCha20-Poly1305 的加密传输，是这个项目自己实现的
...
默认密码是空字符串。此时会使用明文传输，客户端和服务端都会输出警告。设置非空密码后会启用密码认证，以及基于 Argon2id 和 ChaCha20-Poly1305 的加密传输。
